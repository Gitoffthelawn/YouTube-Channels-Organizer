...
async function loadCategoriesAndOrder() {
    try {
        const data = await browser.storage.sync.get(['categories', 'apiKey', 'categoryOrder']);
...
async function saveCategoryOrder() {
    try {
        await browser.storage.sync.set({ categoryOrder: categoryOrder });
...
function renderCategoryContent(categoryName) {
    ...
    deleteCatButton.innerHTML = '🗑️'; // Trash can emoji
...
async function fetchRecentVideos(categoryName, channelIds, container) {
    ...
    try {
        ...
        if (response.error) {
            container.innerHTML = `<p class="error">Error fetching videos: ${response.error}</p>`; return;
        }
        ...
        if(viewToggleButton) viewToggleButton.disabled = false;
        console.error("Error in fetchRecentVideos (categories.js):", error);
...
// --- CRUD Operations (Updated for categoryOrder) ---
...
// --- Export/Import Functions (Updated for categoryOrder) ---
