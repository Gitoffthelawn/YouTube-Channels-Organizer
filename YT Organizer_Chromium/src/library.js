...
async function loadVideos(categoryId) {
  ...
  const res = await chrome.runtime.sendMessage({ type: "GET_VIDEOS_FOR_CATEGORY", categoryId });
...
function renderVideos(videos) {
  ...
  videos.forEach((vid) => {
    ...
    const link = card.querySelector(".video-title");
    link.addEventListener("click", (e) => {
      e.preventDefault();
      openPlayer(vid);
    });
    card.addEventListener("click", (e) => {
      if ((e.target).classList.contains("video-title")) return;
      openPlayer(vid);
    });
  });
}
...
async function persistOrder() {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    await chrome.runtime.sendMessage({ type: "UPDATE_CATEGORY_ORDER", categoryId: id, order: i + 1 });
  }
  await loadCategories();
}
...
function updateTabsNav() {
  // Optional: could disable buttons if at ends.
}

async function ensureChannelName(channelId, currentTitle) {
  if (currentTitle && currentTitle !== "YouTube Creator") return currentTitle;
  const res = await chrome.runtime.sendMessage({ type: "REFRESH_CHANNEL_TITLE", channelId });
...
document.getElementById("export-btn").addEventListener("click", async () => {
  ...
  // Strip video cache from export; only keep categories/channels.
...
document.getElementById("import-file").addEventListener("change", async (e) => {
  ...
  try {
    ...
    const res = await chrome.runtime.sendMessage({ type: "IMPORT_STATE", state: parsed });
    if (res?.error) {
